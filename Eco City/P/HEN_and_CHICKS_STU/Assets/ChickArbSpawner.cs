...
public class ChickArbSpawner : MonoBehaviour {
	...
	void Start () {
		sample =  Resources.Load<GameObject> ("CHICK_ARB");
		if (sample == null)
	...
	void Update () {
		...
		if (elapsedTime >= interval) {
			...
			HenFollowingArbitrated chk = clone.GetComponent<HenFollowingArbitrated> ();
			if (chk == null)
				Debug.LogError ("CHICK_ARB has no HenFollowingArbitrated steering attached");
			chk.target = mommyHen;
