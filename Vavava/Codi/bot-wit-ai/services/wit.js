...
var actions = {
	...
	merge(sessionId, context, entities, message, cb) {
		...
		delete context.tiempo

		var calle = firstEntityValue(entities, 'calle')
		if (calle) {
			context.calle = calle
			delete context.num
		}

		var num = firstEntityValue(entities, 'numero')
		if (num && context.calle) {
			context.num = num
		} else {
			delete context.num
		}
	...
	['addPointRoute'](sessionId, context, cb) {
		//var wantedPics = allPics[context.cat || 'default']
		//context.pics = wantedPics[Math.floor(Math.random() * wantedPics.length)]
		// https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s
		// AIzaSyBHBFVyZct5fhyp1nzHJF3B0d3bEG95mYo

		request('https://maps.googleapis.com/maps/api/geocode/json?address=carrer '+context.calle+ " " + context.num + ", mataro"+'&key=AIzaSyBHBFVyZct5fhyp1nzHJF3B0d3bEG95mYo', function (error, response, body) {
		if (!error && response.statusCode == 200) {
			...
			if (body.status == "OK") {
				...
				request('http://198.245.62.57:7077/add_point?lat='+lat+'&lng='+lng, function (error, response, body) {
	...
	['end'](sessionId, context, cb) {
		delete context.calle
		delete context.num
		cb(context)
	},
